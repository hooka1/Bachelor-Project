...
function AdminAllAssigned({navigation}) {
  ...
  function assignTranslatorDone(id,LanguageFrom,LanguageTo,CancelledBy){
    if (!netInfo.isConnected || !netInfo.isInternetReachable) {
      alert("Please Connect to the Internet")
    }
    else{
        Axios.post('http://10.0.2.2:8000/Reassign',{  
            _id:id
          },{
            headers:{authorization:'Bearer ' +token.token},
          }
          ).then((response)=>{
            if(response.data.message=="Request will be Reassigned"){
              alert("Request will be Reassigned");
              console.log(response.data.rCancelled)  
      navigation.navigate("AssignTranslator",{session:id,LF:LanguageFrom,LT:LanguageTo, CB:response.data.rCancelled})
            }
        }).catch((err) => {
  ...
  function assignTranslator(id,LanguageFrom,LanguageTo,CancelledBy){
    return Alert.alert(
      "Are your sure?",
      "Are you sure you want to reassign the translator for this session?",
      [
        // The "Yes" button
        {
          text: "Yes",
          onPress: () => {
            assignTranslatorDone(id,LanguageFrom,LanguageTo,CancelledBy)          },
        },
        // The "No" button
...
cancelRequestDone(input)          },
  ...
  function loadUserRequests(){
    ...
    Axios.get('http://10.0.2.2:8000/AssignedRequests',{
      headers:{authorization:'Bearer ' +token.token},
    }
    ).then((response)=>{
...
:               
              ...
              <TouchableOpacity onPress={()=>{ assignTranslator(itemData.item._id,itemData.item.LanguageFrom,itemData.item.LanguageTo);}}>
          <Text style={button4} >Reassign</Text>
          </TouchableOpacity>
...
export default AdminAllAssigned;
