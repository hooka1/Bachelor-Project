...
function ClientResolved({navigation}) {
  ...
  function Details(input){
    ...
    if (!netInfo.isConnected || !netInfo.isInternetReachable) {
        alert("Please Connect to the Internet")
          ...
          navigation.navigate("ReportDetails",{details:input})
    }
}
  ...
  function loadUserRequests(){
   ...
   if (!netInfo.isConnected || !netInfo.isInternetReachable) {
    ...
    Axios.get('http://10.0.2.2:8000/myResolvedReports',{
  ...
  return(
    ...
    <FlatList data={reports} renderItem={itemData=>{
      ...
      <Pressable android_ripple={{color:'#4267B3'}}><View style={styles.request}>
        <View style={{width:'50%'}}>
          <Text style={styles.requestText}>Report Responded to at:</Text>
          <Text style={styles.requestText}>{itemData.item.updatedAt}</Text>
          </View>
