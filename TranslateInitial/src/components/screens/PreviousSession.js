...
function PreviousSession({navigation}) {
...
function ViewRequestDetails(input){
  ...
  function loadUserRequests(){
   ...
   if (!netInfo.isConnected || !netInfo.isInternetReachable) {
    ...
    Axios.get('http://10.0.2.2:8000/myPreviousRequests',{
...
:               
      ...
      <Text style={button2}>Details</Text>
          ...
          {itemData.item.Reported==false ?<TouchableOpacity onPress={()=>{ ReportRequest(itemData.item);}}>
      <Text style={button1} >Report</Text>
      </TouchableOpacity>:<></>  }
      </View>
      </View>
